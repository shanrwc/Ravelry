...
void graphUserRatio(string passwd)
{
  gStyle->SetOptStat(0);
  ...
  //Set up TCanvas
  TCanvas* canvas = new TCanvas("user_ratios","user_ratios",14,33,600,600);
  ...
  for (int nYear = 0; nYear != 9; ++nYear)
  {
    ...
    {
      string sql1 = "SELECT COUNT(*) FROM (SELECT user_id,source FROM projects GROUP BY user_id HAVING COUNT(*) > 1) AS sub_projects WHERE source = 'interweave-knits-"+seasons[nSeason]+"-"+makeString(nYear+2005)+"'";
      //      cout << sql << endl;
      result1 = db->Query(sql1.c_str());
      if (result1 == 0) continue;

      string sql2 = "SELECT COUNT(*) FROM (SELECT user_id,source FROM projects GROUP BY user_id HAVING COUNT(*) = 1) AS sub_projects WHERE source = 'interweave-knits-"+seasons[nSeason]+"-"+makeString(nYear+2005)+"'";
      result2 = db->Query(sql2.c_str());
      if (result2 == 0) continue;

      int nRows = result1->GetRowCount();
      int nFields = result1->GetFieldCount();
	...
	row2 = result2->Next();
  	...
  	{
	  double reps = makeDouble(row1->GetField(j));
	  double ones = makeDouble(row2->GetField(j));

	  xes[nYear*4+nSeason] = counter;
	  xerrs[nYear*4+nSeason] = 0.0;
	  yes[nYear*4+nSeason] = reps/ones;
	  yerrs[nYear*4+nSeason] = (reps/ones)*sqrt((1/reps)+(1/ones));
  	}
  ...
  graph->GetYaxis()->SetTitle("Repeat Users/Single Users");
